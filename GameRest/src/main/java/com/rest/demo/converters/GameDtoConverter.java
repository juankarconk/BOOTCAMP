...
@Component
public class GameDtoConverter implements ConverterFactory<Game, GameDto> {
	...
	private final class GameToGameDto<T extends GameDto> implements Converter<Game, GameDto> {
		...
		public GameDto convert(Game source) {
			...
			return new GameDto(source.getIdgames(), source.getName(), source.getDescription(), source.getYear(),
					(ArrayList<GenreEnum>) listgenre);
		}
